...
void setup(){
  ...
  // Load a soundfile
  soundfile = new SoundFile(this, "Inon Zur - Of the People, for the People.mp3");
...
void draw() {
  ...
  float roomSize = map(getX, 0, width, 0, 1.0);
  reverb.room(roomSize);

  // Change the high frequency dampening parameter
  float damping = map(getX, 0, width, 0, 1.0);
  reverb.damp(damping);

  // Change the wet/dry relation of the effect
  float effectStrength = map(getY, 0, height, 0, 1.0);
